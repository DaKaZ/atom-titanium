...
var regExps = {
  ...
  js : [
    // Custom click providers for our view manager library
    {
      regExp : /.*\.openWindow\({[\s\S]*name\s*:\s*['"](.*)["'],?[\s\S]*}\);/gm,
      targetPath : function(text,sourcePath){
        return sourcePath.replace(/app\/(.*)$/,`app/controllers/${text}.js`)
      }
    },
    {
      regExp : /.*\.openAndCloseAllWindows\({[\s\S]*name\s*:\s*['"](.*)["'],?[\s\S]*}\);/gm,
      targetPath : function(text,sourcePath){
        return sourcePath.replace(/app\/(.*)$/,`app/controllers/${text}.js`)
      }
    },
    {
      regExp : /.*\.openAndCloseActiveWindow\({[\s\S]*name\s*:\s*['"](.*)["'],?[\s\S]*}\);/gm,
      targetPath : function(text,sourcePath){
        return sourcePath.replace(/app\/(.*)$/,`app/controllers/${text}.js`)
      }
    },

    // End custom click providers
